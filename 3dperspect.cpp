...
int wmain() {
	...
	while (true) {
			...
			for (float testRay = 0.0f; testRay < 10.0f; testRay += 0.1f) {
				...
				if (isSolid(testRay * castX + playerX, testRay * castY + playerY)) {
					...
					for (int y = floor; y < cheight - floor; y++) {
						console::write((theta + 0.7854f) * step, y, characters[int(testRay)], FRED | BBLACK);
					}
					break;
				}
			}
		...
		for (int x = 0; x < 11; x++) {
			for (int y = 0; y < 11; y++) {
				console::write(x, y, isSolid(x + playerX - 5.5f,y + playerY - 5.5f) ? '#' : ' ', FWHITE | BBLACK);
			}
		}	
		console::write(5,5,'o',FRED|BBLACK);
		console::write(5.5f + sinf(playerA), 5.5f - cosf(playerA), '+', FGREEN|BBLACK);
